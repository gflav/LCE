...
if(DZE_HaloJump) then {
	if(_inVehicle && (_vehicle isKindOf "Air") && ((([_vehicle] call FNC_getPos) select 2) > 200)) then {
	...
	*/
	...
	if (_isMan && !_isPZombie && _traderType in serverTraders) then {
		...
		if (s_player_parts_crtl < 0) then {
			...
			if((_traderMenu select 2) == "messiah") then {
				_humanity_logic = (_humanity < 30000);
			};
			if((_traderMenu select 2) == "harbinger") then {
				_low_high = "high";
				_humanity_logic = (_humanity > -90000);
			};
...
/*
_isBikeRepack = typeOf cursorTarget in ["MMT_Civ"];

if((_isBikeRepack) and _canDo) then {
if (s_player_packbike < 0) then {
        s_player_packbike = player addaction[("<t color=""#007ab7"">" + ("Re-Pack Bike") +"</t>"),"deploys\pack.sqf","",5,false,true,"", ""];
    };
} else {
...
/*
_isZeroFlag = typeOf cursorTarget in ["FlagCarrierBLUFOR_EP1"];

if((_isZeroFlag) and _canDo) then {
if (s_player_hitFlag < 0) then {
        s_player_hitFlag = player addaction[("<t color=""#007ab7"">" + ("Trigger Zero Crate") +"</t>"),"\z\addons\dayz_server\zeros\scripts\ZerosFlagTriggered.sqf","",5,false,true,"", ""];
    };
} else {
    player removeAction s_player_hitFlag;
    s_player_hitFlag = -1;
};
*/
