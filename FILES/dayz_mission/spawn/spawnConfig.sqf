...
_bodyCheck = 3600; // If a player has a body within this distance of a spawn that spawn will be blocked. Set to -1 to disable.
...
_spawnRadius = 900; // Distance around spawn to find a safe pos. Lower is closer to exact coordinates. Do not set too low or BIS_fnc_findSafePos may fail.
_customBase = [
...
_spawnPoints = switch (toLower worldName) do {
	case "chernarus":{
		[
			["Random",[[6901,2509,0],[10294,2191,0],[6901,2509,0],[2236,1923,0]],0,0,1], // Random will never be blocked, so always keep it if using body check.
			...
			["Bandit",[1607,7804,0],0,-5000],
			["Hero",[12944,12767,0],0,5000],
			["Bear Grylls",[3828,11803,0],0,10000],
			["Templar",[6114,9389,0],0,15000],
			["Smuggler",[10002.8,10535.3,0],0,-50000],
			["Marauder",[5763.53,10758.7,0],0,-50000],
			["Harbinger",[12307,14900,0],0,-100000],
			["Messiah",[3700,14537,0],0,30000]
		]
	};
	case "tavi":{
		[
