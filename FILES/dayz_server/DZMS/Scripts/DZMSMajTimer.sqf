...
while {_run} do
{
	...
	//Check for Zero var
	diag_log text format ["[DZMS]: ZERO VERSION %1.",ZEROVersion];
	
	if(!_added) then{
		if(ZEROVersion == 4) then{
			DZMSMajorArray = DZMSMajorArray + ["Zero4"];
			//Added the Zero mission
			diag_log text format ["[DZMS]: Added the Zero mission"];
			_added = true;
		};
	};
	...
	DZMSMajorMissionCounter = DZMSMajorMissionCounter + 1;
	if(DZMSMajorMissionCounter > (_cntMis - 1)) then {
		DZMSMajorMissionCounter = 0;
	};
	
	_varName = DZMSMajorArray select DZMSMajorMissionCounter;
