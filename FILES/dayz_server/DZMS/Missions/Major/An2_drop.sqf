...
//Name of the Mission
_missName = "Building Supply Drop";
...
_hint = parseText format["<t align='center' color='#ff0000' shadow='2' size='1.55'>Building Supply Drop</t><br/><t align='center' color='#ffffff'>An AN-2 with Building Supplies is Flying In! Get to the Location and Capture the Building Supplies!</t>"];
...
while {_loop} do {
	if (!Alive _plane OR !Alive _pilot) then {
		...
		[nil,nil,rTitleText,"The AN2 was shot down by UN Peacekeepers! Go Find the Supplies!", "PLAIN",10] call RE;
...
[[(getpos _boxFin select 0), (getpos _boxFin select 1), 0],4,3,"DZMSUnitsMajor"] call DZMSAISpawn;
sleep 5;
...
// Send message to users
_hint = parseText format["<t align='center' color='#00ff00' shadow='2' size='1.55'>Building Supply Drop</t><br/><t align='center' color='#ffffff'>The AN2 Cargo has been Secured by Survivors!</t>"];
customRemoteMessage = ['hint', _hint];
publicVariable "customRemoteMessage";
