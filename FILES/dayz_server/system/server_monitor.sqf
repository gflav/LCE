...
if (isServer && isNil "sm_done") then {
		...
		if (_damage < 1) then {
			...
			if (_object isKindOf "AllVehicles") then {
                ...
                if (_object isKindOf "AN2_DZ") then {
                    _object addWeapon "BombLauncher"; //M240_veh
					//add two magazines
					_object addMagazine "6Rnd_GBU12_AV8B"; //"100Rnd_762x51_M240"
				...
				if (!((typeOf _object) in dayz_allowedObjects)) then {
					...
					if ((parseNumber _ownerID <= 500000)&& (_ownerID != "0") && !(_object isKindOf "Bicycle")) then{
	...
	// Zeros!	
	[] execVM "z\addons\dayz_server\Zeros\ZerosInit.sqf";
